...
_ghostshell_refresh_pending_proof_fields() {
...
_ghostshell_pending_execution_has_provenance() {
    if [[ -n "$GHOSTSHELL_PENDING_LAST_ACTION" || -n "$GHOSTSHELL_PENDING_ACCEPTED_ORIGIN" ]]; then
        return 0
    fi
    return 1
}
...
_ghostshell_preexec_hook() {
    _ghostshell_session_sign_if_active
    if _ghostshell_pending_execution_has_provenance; then
        _ghostshell_refresh_pending_proof_fields
    else
        _ghostshell_snapshot_pending_execution
    fi
